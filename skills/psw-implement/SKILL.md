...
- 공유 파일(DB 마이그레이션, 라우트 등록, 테마 파일, UI 컴포넌트 코드, 앱 셸 코드. 경로는 `architecture.md` UI 절과 백엔드 절)은 병합 단계에서 오케스트레이터가 순서대로 반영한다
...
- 검증자는 AC 테스트 작성법을 `docs/conventions.md` AC 테스트 절에서, 테스트 위치를 `architecture.md` 백엔드 절에서 읽는다
