...
4. 프로젝트별 결정: 백엔드 키트의 프레임워크, 태그, 헬퍼, 패키지 이름을 사용자에게 정하게 하고 DEC로 남긴다 (API 설계 전)
...
4. `ui/ia.md`에 화면 목록, 메뉴 계층, 화면 흐름을 쓴다
   ...
   - 셸을 쓰지 않는 화면(로그인 등)은 셸 없는 화면 목록에 적는다
   - 메뉴를 관리 기능으로 바꾸는 요구사항이 있으면, 메뉴 계층이 초기 메뉴라는 것과 근거 FR을 적는다
5. 셸을 만든다
   - 형태와 설정(키트 셸 틀의 설정 구역)을 사용자에게 정하게 하고 DEC로 남긴다. `architecture.md` UI 절에 적는다
...
4. `architecture.md`를 채운다
   - 백엔드 절: 키트 태그·헬퍼(DEC), 코드 루트, DB 마이그레이션 경로, 테스트 위치, 사용자 도메인을 구현할 때 고칠 골격 테스트
...
5. 화면마다 `ui/screens/<domain>/<screen>.html` 목업을 만든다
   ...
   - 셸은 `ui/shell.js`를 불러온다. 셸 없는 화면 목록에 있는 화면은 불러오지 않는다
